  ...
  return (
    <div className="mx-auto max-w-4xl space-y-8">
      ...
      <div className="overflow-x-auto rounded-2xl border border-border">
          ...
          <tbody>
            {(rows ?? []).map((r) => (
              <tr key={r.id} className="border-b border-border/50">
                <td className="px-4 py-3 tabular-nums text-muted-foreground">{r.expense_date}</td>
        ...
        {rows?.length === 0 ? (
          <p className="p-6 text-center text-sm text-muted-foreground">Sin gastos registrados.</p>
        ) : null}
      </div>
    </div>
  );
